...
def exeCode(count, uid, ts):
    ...
    for frame in range(count):
        image = cv2.imread(path + '/frames/frame_' +
                           str(frame)+'.png')  # load the image
...
def execPalmCode(count, uid, ts):
    ...
    if len(p2) == 1:
        ...
        pn2 = p2[1]
    local_maxima2 = argrelextrema(
        n[2], np.greater_equal, order=15, mode='clip')
...
def execPalmCode2(count, uid, ts):
    ...
    # ------- Contour Detection -----------
    for frame in range(count):
        im1 = cv2.imread(path + "/frames/frame_" +
                         str(frame)+".png")  # load the image
        r1 = 200.0 / im1.shape[1]
        dim1 = (200, int(im1.shape[0] * r1))
        image = cv2.resize(im1, dim1, interpolation=cv2.INTER_AREA)
...
def execPalmCode2FPS(count, uid, ts):
    path = settings.STORAGE + '/{}/{}'.format(uid, ts)

    # ------- Contour Detection -----------
    # count = 200
    im1 = cv2.imread(path + '/frames/frame_1.jpg')
    r1 = 200.0 / im1.shape[1]
    dim1 = (200, int(im1.shape[0] * r1))
    image = cv2.resize(im1, dim1, interpolation=cv2.INTER_AREA)
    #r2 = 200.0 / imgc.shape[1]
    ...
    frame = []
    res1 = []
    imgr = cv2.imread(path + '/frames/frame_1.jpg', 0)

    for i in range(1, count):
        frame.append(i)
        img = cv2.imread(path + '/frames/frame_'+str(i)+'.jpg')
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    while i < len(y) - window_size + 1:
        ...
        moving_averages.append(window_average)
        i += 1
    np.savetxt(path + "/csvs/m_corr_wir.csv", res1, delimiter=",")
    np.savetxt(path + "/csvs/m_corr_wir_250.csv",
               moving_averages, delimiter=",")

    # -------------polyfit--------------------------
    nos = []

    y1 = pandas.read_csv(path + '/csvs/m_corr_wir_250.csv')
    y2 = y1.values.tolist()
    y = [x[0] for x in y2]
    x = np.array(range(0, len(y)))
    z = np.polyfit(x, y, 6)
    ...
    der = r(x)
    d = r(der)
    np.savetxt(path + "/csvs/m_fit_wir.csv", ab, delimiter=",")

    # ---------time-extract-----------
    nos = []
    y1 = pandas.read_csv(path + '/csvs/m_fit_wir.csv')
    y2 = y1.values.tolist()
    y3 = [x[0] for x in y2]
    a = np.array(y3)
    n = np.array_split(a, 3)
    ...
    istop = pn3
    leng = len(a)
    decolor = (sstart-dstart)/2
    stable = (sstop-sstart)/2
    color = (istop-sstop)/2
    ...
    nos = np.reshape(np.asanyarray(nos), (1, 4))
    np.savetxt(path + "/csvs/m_feature_wir.csv", nos, delimiter=",")
    ...
    reddy1 = np.asanyarray(corr)
    reddy1 = np.reshape(corr, (1, len(reddy1)))
    np.savetxt(path + "/csvs/m_corr_feat_wir.csv", reddy1, delimiter=",")

    # %%%%%%%%%%%
    for i in range(1, 11):
        img = cv2.imread(path + '/frames/frame_'+str(i)+'.jpg')

        imager = cv2.resize(img, dim1, interpolation=cv2.INTER_AREA)

        img = imager[mid_row-50:mid_row+50, mid_col-50:mid_col+50]

        img_red = img[:, :, 2]

        img_avg = np.mean(img_red)
    ...
    np.savetxt(path + "/csvs/m_red_wir.csv", reddy, delimiter=",")
    ...
    all_feat = np.concatenate((nos, reddy1, reddy, reddy2), axis=1)
    np.savetxt(path + "/csvs/m_all_feat_wir.csv", all_feat, delimiter=",")
